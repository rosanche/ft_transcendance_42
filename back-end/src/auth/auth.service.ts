...
export class AuthService {
  ...
  async getUserFromSocket(socket: Socket) {
    const cookie: string =
      socket.handshake.headers.cookie || socket.handshake.auth.token || '';
    if (!cookie) {
      //throw new WsException('cookie missing');
      return null;
    }

    const { access_token: access_token } = socket.handshake.headers.cookie
      ? parse(cookie)
      : { access_token: cookie };
  ...
  async signToken(payload: Partial<TokenPayload>): Promise<{
    access_token: string;
